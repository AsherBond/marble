...
class GnomonicProjectionPrivate : public AbstractProjectionPrivate
{
  public:
...
GnomonicProjection::GnomonicProjection()
    : AbstractProjection( new GnomonicProjectionPrivate( this ) )
{
...
bool GnomonicProjection::repeatableX() const
{
    return false;
}

qreal GnomonicProjection::maxValidLat() const
{
    return +90.0 * DEG2RAD;
}

qreal GnomonicProjection::minValidLat() const
{
    return -90.0 * DEG2RAD;
}
                                             ...
                                             qreal &x, qreal &y, bool &globeHidesPoint ) const
{
    ...
    const qreal cosC = qSin( phi1 ) * qSin( phi ) + qCos( phi1 ) * qCos( phi ) * qCos( lambda - lambdaPrime );
                                          ...
                                          GeoDataCoordinates::Unit unit ) const
{
    ...
    lon = centerLon + qAtan( rx*qSin(c) / ( p*qCos( centerLat )*qCos( c ) - ry*qSin( centerLat )*qSin( c )  ) );
    while ( lon < -M_PI ) lon += 2 * M_PI;
    while ( lon >  M_PI ) lon -= 2 * M_PI;

    lat = qAsin( qCos(c)*qSin(centerLat) + ry*qSin(c)*qCos(centerLat)/p );
...
GeoDataLatLonAltBox GnomonicProjection::latLonAltBox( const QRect& screenRect,
                                                       const ViewportParams *viewport ) const
{
